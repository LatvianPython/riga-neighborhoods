...
def meridional_arc(phi):
    ...
    return (
        111132.95255 * math.degrees(1 * phi)
...
def to_lks92(lat, lng):
    ...
    radius_of_curvature_meridian = (
        semi_major_axis * (1.0 - first_eccentricity)
    ) / math.pow((1.0 - first_eccentricity * math.pow(math.sin(lat), 2.0)), 3.0 / 2.0)

    radius_of_curvature_prime = radius_of_curvature_meridian * (
        1 + second_eccentricity * math.pow(math.cos(lat), 2)
    )
    ...
    terms = {
        "1": (meridional_distance * scale),
        "2": (radius_of_curvature_prime * math.sin(lat) * math.cos(lat) * scale) / 2.0,
        "3": (
            (
                (
                    radius_of_curvature_prime
                    * math.sin(lat)
                    * math.pow(math.cos(lat), 3.0)
                    * scale
                )
                / 24.0
            )
        ...
        "7": (
            ((radius_of_curvature_prime * math.pow(math.cos(lat), 3.0) * scale) / 6.0)
            * (1.0 - tan_sqr + second_eccentricity * math.pow(math.cos(lat), 2.0))
        ),
        "8": (
            ((radius_of_curvature_prime * math.pow(math.cos(lat), 5.0) * scale) / 120.0)
            * (
                5.0
                - 18.0 * tan_sqr
                + tan_sqr * tan_sqr
        ...
        "9": (
            ((radius_of_curvature_prime * pow(math.cos(lat), 7.0) * scale) / 5040.0)
            * (
                61.0
                - 479.0 * tan_sqr
...
if __name__ == "__main__":
    ...
    print(
        "error: {}, {}".format(
            round(result[0] - official[0], 3), round(result[1] - official[1], 3)
        )
    )
